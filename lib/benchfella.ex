defmodule Benchfella do
  ...
  @setup_each_func :"Benchfella.__setup_each__"
  @teardown_each_func :"Benchfella.__teardown_each__"
  ...
  defmacro __using__(_) do
    quote do
      ...
      def unquote(@setup_each_func)(mod_context) do
        {:ok, mod_context}
      end

      defoverridable [{unquote(@setup_each_func), 1}]
    end
  end
  ...
  def run(bench_time, verbose, format, outdir, mem_stats, sys_mem_stats) do
    ...
    {total_time, _, _, last_mod, mod_context} =
      :ets.foldl(&run_bench(&1, &2, verbose, bench_config),
                {0, 1, bench_count, nil, nil},
                @bench_tab)
    run_teardown(last_mod, mod_context)
  ...
  defp run_bench(
    {{mod, func}}, {total_time, i, count, last_mod, old_mod_context}, follow, config)
  do
    {elapsed, mod_context} = :timer.tc(fn ->
      mod_context = if mod != last_mod do
        run_teardown(last_mod, old_mod_context)
      ...
      log_msg = if follow, do: "[#{format_now()}] #{i}/#{count}: #{func}"

      {n, elapsed, mem_stats} =
        run_bench_with_context(mod, mod_context, log_msg, fn context ->
    ...
    {total_time+elapsed, i+1, count, mod, mod_context}
  end

  defp run_bench_with_context(mod, context, log_msg, func) do
    bench_context = run_setup_each(mod, context)
    if log_msg, do: log log_msg
    result = func.(bench_context)
    run_teardown_each(mod, bench_context)
    result
  end
  ...
  defmacro before_each_bench(mod_context, [do: body]) do
    quote do
      def unquote(@setup_each_func)(unquote(mod_context)) do
        unquote(body)
      end
    end
  end
  ...
  defmacro after_each_bench(bench_context, [do: body]) do
    quote do
      def unquote(@teardown_each_func)(unquote(bench_context)) do
        unquote(body)
      end
    end
  end
  ...
  defp run_teardown(mod, mod_context) do
    # TODO: an exception raised here should only abort a single module
  ...
  defp run_setup_each(mod, mod_context) do
    # TODO: an exception raised here should only abort a single test
    if function_exported?(mod, @setup_each_func, 1) do
      case apply(mod, @setup_each_func, [mod_context]) do
  ...
  defp run_teardown_each(mod, bench_context) do
    # TODO: an exception raised here should only abort a single test
    if function_exported?(mod, @teardown_each_func, 1) do
      apply(mod, @teardown_each_func, [bench_context])
    end
  end
